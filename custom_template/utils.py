...
def W_init_by_lora(model, lora_model):
    """
    ...
    with torch.no_grad():
        for i in range(len_of_layers):
            model.encoder.layers[i].self_attention.fc_q.lora_A.copy_(loraA_q_encoder_weight_list[i])
            model.encoder.layers[i].self_attention.fc_q.lora_B.copy_(loraB_q_encoder_weight_list[i])
            model.encoder.layers[i].self_attention.fc_v.lora_A.copy_(loraA_v_encoder_weight_list[i])
            model.encoder.layers[i].self_attention.fc_v.lora_B.copy_(loraB_v_encoder_weight_list[i])

            model.decoder.layers[i].self_attention.fc_q.lora_A.copy_(loraA_q_decoder_weight_list[i])
            model.decoder.layers[i].self_attention.fc_q.lora_B.copy_(loraB_q_decoder_weight_list[i])
            model.decoder.layers[i].self_attention.fc_v.lora_A.copy_(loraA_v_decoder_weight_list[i])
            model.decoder.layers[i].self_attention.fc_v.lora_B.copy_(loraB_v_decoder_weight_list[i])
    ...
    with torch.no_grad():
        for i in range(len_of_layers):
            ...
            model.decoder.layers[i].self_attention.fc_q.weight.copy_(
                (loraA_q_decoder_weight_list[i].T @ loraB_q_decoder_weight_list[i].T).T
            )

            model.decoder.layers[i].self_attention.fc_v.weight.copy_(
                (loraA_v_decoder_weight_list[i].T @ loraB_v_decoder_weight_list[i].T).T
            )
...
def W_init_by_SVD(model, SVD_model, rank):
    ...
    with torch.no_grad():
        for i in range(len_of_layers):
            encoder_q_original_weight = SVD_model.encoder.layers[i].self_attention.fc_q.weight.data.T
            encoder_v_original_weight = SVD_model.encoder.layers[i].self_attention.fc_v.weight.data.T

            decoder_q_original_weight = SVD_model.decoder.layers[i].self_attention.fc_q.weight.data.T
            decoder_v_original_weight = SVD_model.decoder.layers[i].self_attention.fc_v.weight.data.T
    ...
    with torch.no_grad():
        for i in range(len_of_layers):
            model.encoder.layers[i].self_attention.fc_q.lora_A.copy_(w_q_encoder_loraA_weights[i].transpose(0, 1))
            model.encoder.layers[i].self_attention.fc_q.lora_B.copy_(w_q_encoder_loraB_weights[i].transpose(0, 1))

            model.encoder.layers[i].self_attention.fc_v.lora_A.copy_(w_v_encoder_loraA_weights[i].transpose(0, 1))
            model.encoder.layers[i].self_attention.fc_v.lora_B.copy_(w_v_encoder_loraB_weights[i].transpose(0, 1))

            model.decoder.layers[i].self_attention.fc_q.lora_A.copy_(w_q_decoder_loraA_weights[i].transpose(0, 1))
            model.decoder.layers[i].self_attention.fc_q.lora_B.copy_(w_q_decoder_loraB_weights[i].transpose(0, 1))

            model.decoder.layers[i].self_attention.fc_v.lora_A.copy_(w_v_decoder_loraA_weights[i].transpose(0, 1))
            model.decoder.layers[i].self_attention.fc_v.lora_B.copy_(w_v_decoder_loraB_weights[i].transpose(0, 1))
            ...
            # )
            print(
                "SVD model encoder fc_q norm:  ",
                torch.linalg.norm(SVD_model.encoder.layers[i].self_attention.fc_q.weight),
            ...
            print(
                "SVD model decoder fc_v norm:  ",
                torch.linalg.norm(SVD_model.decoder.layers[i].self_attention.fc_v.weight),
                "\n svd model decoder v - loraA@loraB",
                recon_error(
...
def W_init_by_WplusAB(model, lora_model):
    len_of_layers = len(model.encoder.layers)
    loraA_q_encoder_weight_list = []
    loraB_q_encoder_weight_list = []
    ...
    with torch.no_grad():
        for i in range(len_of_layers):
            encoder_q_plus_AB = (
                lora_model.encoder.layers[i].self_attention.fc_q.weight.data.T
                + (loraA_q_encoder_weight_list[i].T @ loraB_q_encoder_weight_list[i].T)
            ).T
            encoder_v_plus_AB = (
                lora_model.encoder.layers[i].self_attention.fc_v.weight.data.T
                + (loraA_v_encoder_weight_list[i].T @ loraB_v_encoder_weight_list[i].T)
            ).T

            decoder_q_plus_AB = (
                lora_model.decoder.layers[i].self_attention.fc_q.weight.data.T
                + (loraA_q_decoder_weight_list[i].T @ loraB_q_decoder_weight_list[i].T)
            ).T
            decoder_v_plus_AB = (
                lora_model.decoder.layers[i].self_attention.fc_v.weight.data.T
                + (loraA_v_decoder_weight_list[i].T @ loraB_v_decoder_weight_list[i].T)
            ).T

            model.encoder.layers[i].self_attention.fc_q.weight.copy_(encoder_q_plus_AB)

            model.encoder.layers[i].self_attention.fc_v.weight.copy_(encoder_v_plus_AB)

            model.decoder.layers[i].self_attention.fc_q.weight.copy_(decoder_q_plus_AB)

            model.decoder.layers[i].self_attention.fc_v.weight.copy_(decoder_v_plus_AB)
